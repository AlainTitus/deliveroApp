...
export default function Normalisation() {
    ...
    const [lien, setLien] = useState('/peraceIllegaux')
    ...
    useEffect(() => {
        if (value == 0) {
            setLien('/peraceIllegaux')
        } else if (value == 1) {
            setLien('/opexIllegaux')
        }
    }, [value])


    useEffect(() => {
        getStorage('illperace').then(result => {
            console.log(result)
            setPeraceStore([...result])
        })
    }, [])
    useEffect(() => {
        getStorage('illopex').then(result => {
            console.log(result)
            setOpexStore([...result])
        })
    }, [])
    ...
    return (
        <View style={styles.main}>
            <Text style={styles.texte}> Illegaux</Text>
