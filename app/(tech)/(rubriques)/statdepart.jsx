...
export default function Statdepart() {
    ...
    const [supDepart, setSupDepart] = useState(0)
    const [supDepartMoyen, setSupDepartMoyen] = useState(0)
    const [supDepartCritique, setSupDepartCritique] = useState(0)
    ...
    const tauxRemplacement = ()=> {
        ...
        if(supDepart !==0){
            const value = ((supDepartMoyen + supDepartCritique)/supDepart)*100
    ...
    const handleSelect = (val) => {
        ...
        getDetailStat(params.key, val, setFiltre, setLoadingLine, setNbrSup)
    }
    ...
    useEffect(() => {
        sumParHauteur(params.key, setDataGraph, setErrorGraph, setLoadingraph)
    }, [])

    useEffect(()=>{
        nbrCollectDepart(params.key)
        .then(res => {
            setSupDepart(res)
        })
    }, [])
    ...
    useEffect(()=>{
        nbrCollectEtatDepart(params.key, "Moyen")
        .then( res => {
            setSupDepartMoyen(res)
        })
    }, [])
    useEffect(()=>{
        nbrCollectEtatDepart(params.key, "Critique")
        .then( res => {
            setSupDepartCritique(res)
        })
    }, [])
    return (
