...
export default function Elagage() {
  ...
  const SaveInStore = async () => {
    if (depart == null || depart == "Choix d'un depart" || depart == "") {
      ...
      const toSave = { depart, nombre: base, observations, latitude: params.lat, longitude: params.long }

      const listKeyStore = await AsyncStorage.getAllKeys();

      if (listKeyStore.includes("elagage")) {
        const dataStored = await AsyncStorage.getItem('elagage')
        let dataTransform = JSON.parse(dataStored)
        dataTransform.push(toSave)
        await AsyncStorage.removeItem('elagage')
        await AsyncStorage.setItem('elagage', JSON.stringify(dataTransform))
      } else {
        const firstData = [toSave]
        await AsyncStorage.setItem('elagage', JSON.stringify(firstData))
      }
      showToast();
      navigation.goBack();
    }
