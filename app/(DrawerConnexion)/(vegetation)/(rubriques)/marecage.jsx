...
export default function Marecage() {
  const [depart, setDepart] = useState(null)
  const [nomMarecage, setNomMarecage] = useState(null)
  ...
  const SaveInStore = async () => {
    ...
    if (depart == null || depart == "Choix d'un depart" || depart == "") {
      ...
      const toSave = { depart, longueur: base, nom_Mare: nomMarecage, observations, latitude: params.lat, longitude: params.long }

      const listKeyStore = await AsyncStorage.getAllKeys();

      if (listKeyStore.includes("marecage")) {
        const dataStored = await AsyncStorage.getItem('marecage')
        let dataTransform = JSON.parse(dataStored)
        dataTransform.push(toSave)
        await AsyncStorage.removeItem('marecage')
        await AsyncStorage.setItem('marecage', JSON.stringify(dataTransform))
      } else {
        const firstData = [toSave]
        await AsyncStorage.setItem('marecage', JSON.stringify(firstData))
      }
      showToast();
      navigation.goBack();
    }
  ...
  return (
    <View style={{ flex: 1, paddingHorizontal: 8 }}>
      ...
      >
        ...
        />
        <CustomInput
          label="Nom du marécage"
          placeholder={"."}
          labelColor="#1e8449"
          handleInput={setNomMarecage}
          val
        />
        <CustomInput
          label="Longueur du marécage en m"
          placeholder={"."}
          labelColor="#1e8449"
          handleInput={setBase}
          type={"numeric"}
