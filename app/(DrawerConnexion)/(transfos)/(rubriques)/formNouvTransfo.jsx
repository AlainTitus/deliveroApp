...
export default function FormCollecte() {
    const [depart, setdepart] = useState(null);
    const [nomPoste, setNomPoste] = useState(null)
    ...
    const handleSave = async () => {
        ...
        if (listKeyStore.includes("transformateur")) {
            const dataStored = await AsyncStorage.getItem('transformateur')
            let dataTransform = JSON.parse(dataStored)
            dataTransform.push(toSave)
            await AsyncStorage.removeItem('transformateur')
            await AsyncStorage.setItem('transformateur', JSON.stringify(dataTransform))
            // console.log('Données sauvegardées = ', dataTransform)

            navigation.goBack()
        } else {
            const firstData = [toSave]
            await AsyncStorage.setItem('transformateur', JSON.stringify(firstData))
